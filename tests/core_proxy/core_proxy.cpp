...
// 
// 
// BOOST_CLASS_VERSION(nodetool::node_server<currency::t_currency_protocol_handler<tests::proxy_core> >, 1);
// 
...
//   nodetool::node_server<currency::t_currency_protocol_handler<tests::proxy_core> >::init_options(desc);
// 
//   po::variables_map vm;
...
//   currency::t_currency_protocol_handler<tests::proxy_core> cprotocol(pr_core, NULL);
//   nodetool::node_server<currency::t_currency_protocol_handler<tests::proxy_core> > p2psrv(cprotocol);
//   cprotocol.set_p2p_endpoint(&p2psrv);
//   //pr_core.set_currency_protocol(&cprotocol);
...
// 
// 
//   //pr_core.set_currency_protocol(NULL);
...
// 
// /*
// string tx2str(const currency::transaction& tx, const currency::hash256& tx_hash, const currency::hash256& tx_prefix_hash, const currency::blobdata& blob) {
//     stringstream ss;
// 
//     ss << "{" << endl;
...
// }*/
// 
// bool tests::proxy_core::handle_incoming_tx(const currency::blobdata& tx_blob, currency::tx_verification_context& tvc, bool keeped_by_block) {
//     if (!keeped_by_block)
//         return true;
// 
...
// }
// 
// bool tests::proxy_core::handle_incoming_block(const currency::blobdata& block_blob, currency::block_verification_context& bvc, bool update_miner_blocktemplate) {
...
//         while (m_hash2blkidx.end() != cit && currency::null_hash != cit->second.blk.prev_id && n > 0) {
//             n--;
...
// }
// 
// bool tests::proxy_core::add_block(const crypto::hash &_id, const crypto::hash &_longhash, const currency::block &_blk, const currency::blobdata &_blob) {
//     size_t height = 0;
// 
//     if (currency::null_hash != _blk.prev_id) {
