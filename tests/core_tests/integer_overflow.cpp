...
namespace
{
  ...
  void append_tx_source_entry(std::vector<currency::tx_source_entry>& sources, const transaction& tx, size_t out_idx)
  {
    ...
    se.outputs.push_back(currency::make_output_entry(0, boost::get<currency::txout_to_key>(tx.vout[out_idx].target).key));
    se.real_output = 0;
...
bool gen_uint_overflow_1::generate(std::vector<test_event_entry>& events) const
{
  ...
  MAKE_TX_LIST_START(events, txs_0, miner_account, bob_account, TOTAL_MONEY_SUPPLY, blk_2);
  MAKE_TX_LIST(events, txs_0, miner_account, bob_account, TOTAL_MONEY_SUPPLY, blk_2);
...
bool gen_uint_overflow_2::generate(std::vector<test_event_entry>& events) const
{
  ...
  if (!construct_tx(miner_account.get_keys(), sources, destinations, tx_1, txkey, 0))
    return false;
  ...
  if (!construct_tx(bob_account.get_keys(), sources, destinations, tx_2, txkey, 0))
    return false;
