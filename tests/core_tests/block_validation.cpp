...
bool gen_block_miner_tx_has_2_in::generate(std::vector<test_event_entry>& events) const
{
  ...
  se.outputs.push_back(currency::make_output_entry(0, boost::get<txout_to_key>(blk_0.miner_tx.vout[0].target).key));
  se.real_output = 0;
...
bool gen_block_miner_tx_with_txin_to_key::generate(std::vector<test_event_entry>& events) const
{
  ...
  se.outputs.push_back(currency::make_output_entry(0, boost::get<txout_to_key>(blk_1.miner_tx.vout[0].target).key));
  se.real_output = 0;
