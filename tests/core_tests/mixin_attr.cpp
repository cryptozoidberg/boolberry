...
bool mix_attr_tests::generate(std::vector<test_event_entry>& events) const
{
  ...
  MAKE_TX_MIX(events, tx_0, bob_account, alice_account, MK_TEST_COINS(5) - TESTS_DEFAULT_FEE, 3, blk_4);    
  ...
  MAKE_TX_LIST_MIX_ATTR(events, txs_blk_6, miner_account, bob_account2, MK_TEST_COINS(5), blk_5, 4);
  MAKE_TX_LIST_MIX_ATTR(events, txs_blk_6, miner_account, bob_account2, MK_TEST_COINS(5), blk_5, 4);
  MAKE_NEXT_BLOCK_TX_LIST(events, blk_6, blk_5, miner_account, txs_blk_6);
  
  DO_CALLBACK(events, "remember_last_block");                                                           
  MAKE_TX_MIX(events, tx_3, bob_account2, alice_account2, MK_TEST_COINS(5) - TESTS_DEFAULT_FEE, 2, blk_6);
  MAKE_NEXT_BLOCK_TX1(events, blk_7_f, blk_6, miner_account, tx_3);                                      
  DO_CALLBACK(events, "check_last_not_changed");                                                                    

  MAKE_TX_MIX(events, tx_4, bob_account2, alice_account2, MK_TEST_COINS(5) - TESTS_DEFAULT_FEE, 3, blk_6);
