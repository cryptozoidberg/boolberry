...
public:
  ...
  bool init()
  {
    ...
    for (size_t i = 0; i < ring_size; ++i)
    {
      ...
      output_entries.push_back(currency::make_output_entry(i, tx_out.key));
...
protected:
  currency::account_base m_miners[ring_size];
  currency::transaction m_miner_txs[ring_size];
  uint64_t m_source_amount;

  std::vector<currency::tx_source_entry> m_sources;
