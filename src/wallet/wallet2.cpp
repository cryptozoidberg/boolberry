...
//----------------------------------------------------------------------------------------------------
bool wallet2::is_coin_age_okay_for_pos(const transfer_details& tr)
{
...
bool wallet2::get_pos_entries(currency::COMMAND_RPC_SCAN_POS::request& req)
{
  for (size_t i = 0; i != m_transfers.size(); i++)
  {
    ...
    if (tr.m_spent)
      continue;
    if (!is_coin_age_okay_for_pos(tr))
      continue;
...
void wallet2::cancel_offer_by_id(const crypto::hash& tx_id, uint64_t of_ind, currency::transaction& res_tx)
{
  ...
  std::vector<currency::extra_v> extra;
  std::vector<currency::attachment_v> attachments;
  currency::cancel_offer co = AUTO_VAL_INIT(co);
  ...
  blobdata sig_blob = currency::make_offer_sig_blob(co);
