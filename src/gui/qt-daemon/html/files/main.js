...
function init_btc_exchange_rate()
{
    ...
    var res = secure_request_url("https://www.bitstamp.net/api/ticker/", JSON.stringify(req_arams));
    //debug case
    if(res === undefined)
        return;
...
function secure_request_url(url, params) {
    if (window['Qt_parent'] && window['Qt_parent'] !== undefined) {
        return Qt_parent.request_uri(url, params);
    }
    else
    {
        //test data set
...
services['poloniex'] = {
    ...
    fetch: function() {
        ...
        var data = $.parseJSON( request_polo(JSON.stringify(req_arams)));
        ...
        window.setTimeout("services['poloniex'].fetch()", 1000000);
    },
