...
{
    var seed_phrase = backend.get_seed_text();
    if (seed_phrase.length === 0)
        return;

    var wallet_path = backend.browse_wallet(false);
    if (wallet_path.length === 0)
        return;

    var pass = backend.get_password();
    if (pass.length === 0)
        return;

    var r = backend.restore_wallet(seed_phrase, pass, wallet_path);
    if(!r) 
    {
...
function on_switch_view(swi)
{
    ...
    {
        backend.message_box("Wrong view at on_switch_view");
    }
}

function on_close_wallet()
{
    backend.close_wallet();
}

function on_copy_wallet () {
    var $addressValue = $('#wallet_address').text();
    backend.place_to_clipboard($addressValue);
    show_hide_tooltip();
}
...
function on_sign() {
  var sign_res_str = backend.sign_text($('#sign_text_id').val());
...
function on_transfer()
{
  ...
  backend.parse_transfer_target(addressInputVal, function(callback) {
    ...
    //callback will be invoked when parse_transfer_target has a return value
    result = callback;
    ...
    {
      backend.message_box("Wrong transaction lock time specified.");
      return;
    }
    ...
    {
      backend.message_box("Wrong Mixin parameter value, please set values in range 0-10");
      return;
    }    
     
    backend.transfer(JSON.stringify(transfer_obj), function(callback) {
      result = callback;
      var transfer_res_obj = jQuery.parseJSON(result);
      ...
      if(transfer_res_obj.success)
      {
        ...
        backend.message_box("DEBUG: !transfer_res_obj.success");
        return;
      }
    });     
  });    
...
{ // DOM ready
  ...
  //create a channel co communicate with C++ side from JS code and get the QObject
  new QWebChannel(window.qt.webChannelTransport, function (channel) {
    
    //callbacks will be invoked whenever the signal is emitted on the C++/QML side
    channel.objects.backend.update_daemon_state.connect(str_to_obj.bind({cb: on_update_daemon_state}));
    channel.objects.backend.update_wallet_status.connect(str_to_obj.bind({cb: on_update_wallet_status}));
    channel.objects.backend.update_wallet_info.connect(str_to_obj.bind({cb: on_update_wallet_info}));
    channel.objects.backend.money_transfer.connect(str_to_obj.bind({cb: on_money_transfer}));

    channel.objects.backend.show_wallet.connect(show_wallet);
    channel.objects.backend.hide_wallet.connect(hide_wallet);
    channel.objects.backend.switch_view.connect(on_switch_view);
    channel.objects.backend.set_recent_transfers.connect(str_to_obj.bind({cb: on_set_recent_transfers}));
    channel.objects.backend.handle_internal_callback.connect(on_handle_internal_callback);
    ...
    disable_tab(document.getElementById('wallet_view_menu'), true);
    $("#version_text").text(backend.get_version());
    $("#version_text").text(backend.get_version());
});

function secure_request_url(url, params, callback_name) {
    if (window['backend'] && window['backend'] !== undefined) {
        return backend.request_uri(url, params, callback_name);
    }
    else
    {
