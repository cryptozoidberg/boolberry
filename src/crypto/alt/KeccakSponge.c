...
void AbsorbQueue2(spongeState *state, const UINT64* pscratchpd, UINT64 pscratchpd_sz)
{
...
int Absorb(spongeState *state, const unsigned char *data, unsigned long long databitlen, const UINT64* pscratchpd, UINT64 pscratchpd_sz)
{
        ...
        if ((state->bitsInQueue == 0) && (databitlen >= state->rate) && (i <= (databitlen-state->rate))) {
            ...
            if (state->rate == 1088) {
                for(j=0; j<wholeBlocks; j++, curData+=1088/8) {
                    ...
                    displayBytes(1, "Block to be absorbed", curData, state->rate/8);
                    #endif
                    KeccakAbsorb1088bits(state->state, curData, pscratchpd, pscratchpd_sz);
                }
            }
            else
#endif
            ...
            if (state->bitsInQueue == state->rate)
                AbsorbQueue2(state, pscratchpd, pscratchpd_sz);
            if (partialByte > 0) {
