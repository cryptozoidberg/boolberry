  ...
  for (const auto& td : transfers)
  {
    if (!filter || available != td.m_spent)
    {
      ...
      message_writer(td.m_spent ? epee::log_space::console_color_magenta : epee::log_space::console_color_green, false) <<
        ...
        get_transaction_hash(td.m_tx) << "[" << td.m_block_height << "] unlocked: " << (m_wallet->is_transfer_unlocked(td) ? 'T' : 'F');
    }
  }
...
bool simple_wallet::transfer(const std::vector<std::string> &args_)
{
  ...
  try
  {
    ...
    m_wallet->transfer(dsts, fake_outs_count, 0, m_wallet->get_core_runtime_config().tx_pool_min_fee, extra, attachments, tx);
